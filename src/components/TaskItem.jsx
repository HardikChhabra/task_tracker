      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3 flex-1">
          ...
          <div className="flex-1">
            <h4 className={`text-lg font-medium ${task.completed ? 'text-neutral-500 line-through' : 'text-neutral-200'}`}>
              {task.title}
            </h4>
            {task.description && (
        ...
        <div className="flex gap-2 flex-shrink-0">
          ...
          >
            <Edit2 className="text-neutral-400 hover:text-neutral-300" size={18} />
          </button>
          <button
            onClick={() => onDelete(task.id)}
            className="p-2 hover:bg-neutral-700 rounded transition-colors"
          >
            <Trash2 className="text-neutral-400 hover:text-neutral-300" size={18} />
          </button>
        </div>
      </div>
    </div>
